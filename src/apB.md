...
Recall in [Chapter 9 that we talked about functors](chapter_9.md/#a-word-functors): a value along with a map-like utility to perform an operation on all its constitute data members. A monad is a functor that includes some additional behavior.
...
Let's look first at the monadic `map(..)` function. Like `map(..)` on an array (see [Chapter 9](chapter_9.md/#map)) that calls a mapper function with its value(s) and produces a new array, a monad's `map(..)` calls a mapper function with the monad's value, and whatever is returned is wrapped in a new Just monad instance:
...
To connect this `chain(..)` method conceptually to stuff we've already learned, we'll point out that many monad implementations name this method `flatMap(..)`. Now, recall from [Chapter 9 what `flatMap(..)`](chapter_9.md/#user-content-flatmap) does (as compared to `map(..)`) with an array:
...
One way to illustrate `chain(..)` in this manner is in combination with the `identity(..)` utility (see [Chapter 3](chapter_3.md/#one-on-one)), to effectively extract a value from a monad:
...
To make a monad from `A` that holds a value-containing function, we call `A.map(..)`, giving it a curried function that "remembers" that extracted value (see [Chapter 3](chapter_3.md/#one-at-a-time)) as its first argument. We'll call this new function-containing monad `C`:
